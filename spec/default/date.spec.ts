...
test('Quoted date value should parse', t => {
    t.deepEqual(decoratedJson.type(Date).rawToInstance('1543915254'), new Date(1543915254));
    t.deepEqual(decoratedJson.type(Date).rawToInstance('-1543915254'), new Date(-1543915254));
    t.deepEqual(
        decoratedJson.type(Date).rawToInstance('"1970-01-18T20:51:55.254Z"'),
        new Date(1543915254),
    );
});
...
test('Quoted date value should convert to JSON', t => {
    t.is(
        decoratedJson.type(Date).instanceToRaw(new Date(1543915254)),
    ...
    t.is(
        decoratedJson.type(Date).instanceToRaw(new Date('2018-12-04T09:20:54')),
        `"${new Date('2018-12-04T09:20:54').toISOString()}"`,
    );
});

test('Unquoted date should convert from JSON', t => {
    t.deepEqual(decoratedJson.type(Date).plainToInstance(1543915254), new Date(1543915254));
});

test('Unquoted date should convert to JSON', t => {
    const dateMs = new Date(1543915254);
    t.is(decoratedJson.type(Date).instanceToPlain(dateMs), dateMs);
    t.true(decoratedJson.type(Date).instanceToPlain(dateMs) instanceof Date);
    const dateStr = new Date('2018-12-04T09:20:54');
    t.is(decoratedJson.type(Date).instanceToPlain(dateStr), dateStr);
});
...
test('Parsing a date with a date as source value should keep the source value', t => {
    const date = new Date();
    t.is(decoratedJson.type(DateSpec).plainToInstance({date: date}).date, date);
});
...
test('Parsing a date with a timestamp as source value should succeed', t => {
    t.deepEqual(decoratedJson.type(DateSpec).plainToInstance({date: timestamp2000}).date, date2000);
});

test('Parsing a date with a decimal as source value should error', t => {
    t.throws(() => decoratedJson.type(DateSpec).plainToInstance({date: 500000.555}).date, {
...
test('Parsing a date with a boolean as source value should error', t => {
    t.throws(() => decoratedJson.type(DateSpec).plainToInstance({date: true}).date, {
        message: getDiagnostic('invalidValueError', {
            path: 'DateSpec.date',
            expectedType: 'a string (ISO-8601) or integer (time since epoch in ms)',
            actualType: 'Boolean',
...
test('Parsing a date with a negative timestamp as source value should succeed', t => {
    t.deepEqual(
        decoratedJson.type(DateSpec).plainToInstance({date: -timestamp2000}).date,
        new Date(-timestamp2000),
    );
});

test('Parsing a date with an ISO 8601 string as source value should succeed', t => {
    t.deepEqual(decoratedJson.type(DateSpec).plainToInstance({date: iso2000}).date, date2000);
});
